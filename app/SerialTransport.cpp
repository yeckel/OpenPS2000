...
void SerialTransport::run()
{
    ...
    if (m_stopFlag.loadAcquire()) { port.close(); return; }

    // Emit device info to backend.
    emit deviceInfoReady(m_deviceInfo);
    emit statusMessage(QString("Device: %1 — %.1fV / %.1fA / %.0fW")
        .arg(m_deviceInfo.deviceType)
        .arg(static_cast<double>(m_deviceInfo.nomVoltage))
        .arg(static_cast<double>(m_deviceInfo.nomCurrent))
        .arg(static_cast<double>(m_deviceInfo.nomPower)));
    ...
    while (!m_stopFlag.loadAcquire()) {
        ...
        {
            QMutexLocker lock(&m_cmdMutex);
            if (!m_cmdQueue.isEmpty()) cmd = m_cmdQueue.dequeue();
        }

        if (!cmd.isEmpty()) {
            bool ok = sendAndAck(static_cast<void*>(&port), cmd);
            if (!ok) emit statusMessage("Command error or timeout");
...
bool SerialTransport::sendAndAck(void* vport, const QByteArray& telegram) const
{
    ...
    port->clear();
    port->write(telegram);
    if (!port->waitForBytesWritten(200)) return false;
