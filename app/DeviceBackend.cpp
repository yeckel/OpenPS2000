...
void DeviceBackend::disconnectDevice()
{
    ...
    emit statusMessage(QString("Disconnected — %1 samples, %.3f Wh")
        .arg(m_readings.size())
...
{
    if (m_transport) m_transport->enqueueCommand(telegram);
}
...
{
    if (m_deviceInfo.nomVoltage <= 0) return;
...
{
    if (m_deviceInfo.nomCurrent <= 0) return;
...
void DeviceBackend::exportCsv(const QString& rawPath)
{
    ...
    if (m_readings.isEmpty()) { emit statusMessage("No data to export."); return; }

    QFile f(path);
    if (!f.open(QIODevice::WriteOnly | QIODevice::Text)) {
        emit statusMessage("Cannot open file: " + path); return;
    }
    QTextStream out(&f);
...
void DeviceBackend::exportExcel(const QString& rawPath)
{
    ...
    if (!writer.save(path)) {
        emit statusMessage("Excel export failed: " + writer.lastError()); return;
    }
