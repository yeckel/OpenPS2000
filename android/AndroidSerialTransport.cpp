...
AndroidSerialTransport::AndroidSerialTransport(QObject *parent)
    : QObject(parent)
{
    // Move ourselves and a poll timer to a dedicated thread
    moveToThread(&m_thread);

    m_pollTimer = new QTimer();
    m_pollTimer->setInterval(250); // 4 Hz
    m_pollTimer->moveToThread(&m_thread);

    connect(&m_thread, &QThread::started, m_pollTimer, [this]() {
        connect(m_pollTimer, &QTimer::timeout, this, &AndroidSerialTransport::onPollTimer);
    });

    m_thread.setObjectName("AndroidSerial");
    m_thread.start();
}

AndroidSerialTransport::~AndroidSerialTransport()
{
    close();
    m_thread.quit();
    m_thread.wait(2000);
...
QStringList AndroidSerialTransport::availablePorts() const
{
    QJniObject ctx = QNativeInterface::QAndroidApplication::context();
    if (!ctx.isValid()) return {};
...
void AndroidSerialTransport::open(const QString& deviceName)
{
    QJniObject ctx = QNativeInterface::QAndroidApplication::context();
    if (!ctx.isValid()) {
        emit errorOccurred("No Android context");
        return;
    }

    QJniObject inst = QJniObject::callStaticMethod<QJniObject>(
        JAVA_CLASS, "getInstance", "()Lorg/openps2000/UsbSerial;");
    if (!inst.isValid()) {
        emit errorOccurred("UsbSerial.getInstance() failed");
        return;
    }

    QJniObject jName = QJniObject::fromString(deviceName);
    jboolean ok = inst.callMethod<jboolean>(
        "open",
        "(Landroid/content/Context;Ljava/lang/String;)Z",
        ctx.object<jobject>(),
        jName.object<jstring>());
    ...
    if (!ok) {
        emit errorOccurred("Failed to open USB device: " + deviceName);
        return;
    }

    m_open       = true;
    m_deviceName = deviceName;
    QMetaObject::invokeMethod(m_pollTimer, "start", Qt::QueuedConnection);
    emit portOpened(deviceName);
}

void AndroidSerialTransport::close()
{
    if (!m_open) return;
    ...
    QJniObject inst = QJniObject::callStaticMethod<QJniObject>(
        JAVA_CLASS, "getInstance", "()Lorg/openps2000/UsbSerial;");
    if (inst.isValid())
        inst.callMethod<void>("close");

    m_open = false;
    emit portClosed();
}

bool AndroidSerialTransport::isOpen() const
{
    return m_open;
}
...
void AndroidSerialTransport::enqueueCommand(const QByteArray& telegram)
{
    ...
    // Coalesce: if the same OBJ byte is already queued, replace it.
    if (telegram.size() >= 3) {
        const char obj = telegram[2];
        for (auto& item : m_normalQueue) {
            if (item.size() >= 3 && item[2] == obj) {
                item = telegram; // replace in-place
                return;
            }
        }
    }
    m_normalQueue.enqueue(telegram);
}

void AndroidSerialTransport::enqueueUrgent(const QByteArray& telegram)
{
    QMutexLocker lock(&m_queueMutex);
    m_urgentQueue.enqueue(telegram);
}

// ── Poll timer ────────────────────────────────────────────────────────────

void AndroidSerialTransport::onPollTimer()
{
    if (!m_open) return;
    flushAndPoll();
}

void AndroidSerialTransport::flushAndPoll()
{
    ...
    while (true) {
        ...
        {
            QMutexLocker lock(&m_queueMutex);
            if (m_urgentQueue.isEmpty()) break;
            cmd = m_urgentQueue.dequeue();
        }
        sendReceive(cmd); // fire-and-forget for control commands
    }

    // Drain normal queue (one command per timer tick to match desktop behaviour)
    {
        QMutexLocker lock(&m_queueMutex);
        if (!m_normalQueue.isEmpty()) {
            QByteArray cmd = m_normalQueue.dequeue();
            lock.unlock();
            QByteArray reply = sendReceive(cmd);
            if (!reply.isEmpty())
                emit dataReceived(reply);
            return;
        }
    }
}

// ── Low-level I/O via JNI ─────────────────────────────────────────────────

QByteArray AndroidSerialTransport::sendReceive(const QByteArray& cmd)
{
    QJniObject inst = QJniObject::callStaticMethod<QJniObject>(
        JAVA_CLASS, "getInstance", "()Lorg/openps2000/UsbSerial;");
    ...
    // Write
    QJniEnvironment env;
    jbyteArray jCmd = env->NewByteArray(cmd.size());
    env->SetByteArrayRegion(jCmd, 0, cmd.size(),
                            reinterpret_cast<const jbyte*>(cmd.constData()));

    jint written = inst.callMethod<jint>("write", "([B)I", jCmd);
    env->DeleteLocalRef(jCmd);

    if (written < 0) {
        qWarning() << "[AndroidSerial] write failed:" << written;
        m_open = false;
        emit portClosed();
        return {};
    }

    // Read reply (PS2000 reply is small — 16 bytes max)
    constexpr int MAX_REPLY = 64;
    jbyteArray jBuf = env->NewByteArray(MAX_REPLY);
    jint got = inst.callMethod<jint>("read", "([BI)I", jBuf, MAX_REPLY);

    QByteArray reply;
    if (got > 0) {
        reply.resize(got);
        env->GetByteArrayRegion(jBuf, 0, got,
                                reinterpret_cast<jbyte*>(reply.data()));
    }
    env->DeleteLocalRef(jBuf);

    if (got < 0) {
        qWarning() << "[AndroidSerial] read failed — device disconnected?";
        m_open = false;
        emit portClosed();
    }
